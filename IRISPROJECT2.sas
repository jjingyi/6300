...
PROC MEANS NOPRINT data=AA;
 VAR Y1-Y4;
 OUTPUT OUT=B MEAN=MY1-MY4 STD=SY1-SY4;
RUN;
DATA B; SET B;
 DO I=1 TO _FREQ_;
 OUTPUT; END;

DATA BOTH; MERGE AA B;
 NY1=(Y1-MY1)/SY1;
 NY2=(Y2-MY2)/SY2;
 NY3=(Y3-MY3)/SY3;
 NY4=(Y4-MY4)/SY4;
...
PROC ANOVA DATA=BOTH; CLASS GRP;
 MODEL NY1-NY4=GRP;
 MANOVA H=GRP/PRINTH PRINTE;
 MEANS GRP;
RUN;
...
PROC GPLOT DATA=AA;
 PLOT Y3*Y4=GRP;
RUN;
proc discrim data=aa out=b;class grp;
var Y1-Y4;
run;
proc print data=b;
run;

proc candisc data=aa out=c;class grp;
var y1-y4;
run;
proc gplot data=c;
plot can1*can2=grp;
